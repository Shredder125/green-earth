...
function renderCart() {
  ...
  cart.forEach((item, index) => {
    ...
    li.className = "flex justify-between items-center bg-green-100 px-3 py-2 rounded-md"
    li.innerHTML = `
      <span>${item.name}</span>
      <button class="text-red-500 font-bold"><i class="fa-solid fa-xmark"></i></button>
    `
...
function addToCart(plant) {
  cart.push({ name: plant.name, price: plant.price })
  renderCart()
}
